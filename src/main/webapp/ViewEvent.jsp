...
document.addEventListener("DOMContentLoaded", function () {
    let btn = document.getElementById("viewAuditoriumsBtn");
    if (btn) {
        btn.addEventListener("click", function (event) {
            ...
            fetch("ViewAuditoriumServlet1")
                ...
                .then(data => {
                    ...
                    let auditoriumList = document.getElementById("auditoriumList");
                    if (auditoriumList) {
                        auditoriumList.innerHTML = data;
                    } else {
                        console.error("Error: Element with ID 'auditoriumList' not found.");
                    }
                })
                .catch(error => {
                    ...
                    let auditoriumList = document.getElementById("auditoriumList");
                    if (auditoriumList) {
                        auditoriumList.innerHTML = `<p style='color:red;'>Failed to load auditorium list: ${error.message}</p>`;
                    }
                });
        });
    } else {
        console.error("Error: Button with ID 'viewAuditoriumsBtn' not found.");
    }
});
...
<body>
    ...
    <div class="adminsidebar">
        ...
        <a href="ViewEvent.jsp" id="viewEventBtn" data-target="viewEvent">View Event</a>
    </div>
    <main>
...
<table border="1">
    ...
    <tbody>
        <%
            try {
                ...
                while (rs.next()) {
                    ...
                    String registration_type = rs.getString("registration_type");
                    int registration_limit = rs.getInt("registration_limit");
        ...
        <tr>
            ...
            <td><%= registration_limit %></td>
              <td>
                     <a href="EditEvent.jsp?event_id=<%= event_id %>">Edit</a>
        ...
        <%
                ...
                e.printStackTrace();
        %>
        <tr><td colspan="14" style="color: red;">Error loading events.</td></tr>
        <%
            }
        %>
    </tbody>
...
function loadEditForm(eventId) {
    ...
    let url = "EditEvent.jsp?event_id=" + encodeURIComponent(eventId);
    ...
    fetch(url)
        ...
        .catch(error => {
            ...
            document.getElementById("eventContent").innerHTML = "<p>Error loading edit form.</p>";
        });
}
    ...
    function confirmDelete(event_id) {
        if (confirm("Do you really want to delete this event?")) {
            fetch("DeleteEventServlet?event_id=" + event_id, {
                method: "GET"
            })
            .then(() => loadEventList()) // Reload event list after deletion
